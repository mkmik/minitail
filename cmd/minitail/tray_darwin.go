...
func runTray(ctx context.Context, cancel context.CancelFunc, ctrl *app.Controller) error {
	...
	onReady := func() {
		...
		mLogin.Hide()
		mApprove := systray.AddMenuItem("Approve exit node…", "Open the admin console to approve this exit node")
		mApprove.Hide()
		mCopyIP := systray.AddMenuItem("Copy tailnet IP", "Copy this node's tailnet address")
		mCopyIP.Hide()
		systray.AddSeparator()

		mStart := systray.AddMenuItem("Start", "Start the exit node")
		mStop := systray.AddMenuItem("Stop", "Stop the exit node")
		...
		systray.AddSeparator()
		mQuit := systray.AddMenuItem("Quit minitail", "Stop the exit node and quit")
